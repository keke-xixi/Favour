...
// 使用 requestAnimationFrame 优化跳跃动画
...
// 使用 requestAnimationFrame 优化二段跳跃动画
const jumpSecondOnceTime = (originRole?:any) => {
    if(Math.abs(role.y - originRole.y) >= role.jumpHeight) {
    ...
    role.jumpYDirection === 'top' ? role.y += role.jumpYSpeed / FPS.value : role.y -= role.jumpYSpeed / FPS.value;
    role.jumpXDirection === 'right' ? role.x += role.jumpXSpeed / FPS.value : role.x -= role.jumpXSpeed / FPS.value;
    role.requestAnimationId = requestAnimationFrame(() => jumpSecondOnceTime(originRole));
}
...
// 使用 requestAnimationFrame 优化下落动画
const downOnceTime = (originRole?:any) => {
    ...
    role.jumpYDirection === 'top' ? role.y += role.jumpYSpeed / FPS.value : role.y -= role.jumpYSpeed / FPS.value;
    role.jumpXDirection === 'right' ? role.x += role.jumpXSpeed / FPS.value : role.x -= role.jumpXSpeed / FPS.value;
    role.requestAnimationId = requestAnimationFrame(() => downOnceTime(originRole));
}
