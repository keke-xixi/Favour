...
onMounted(async () => {
    ...
    const modules7 = import.meta.glob('./components/js/*.vue');  // js
    for (const path in modules7) {
    ...
    const modules9 = import.meta.glob('./components/uniapp/*.vue');  // uniapp
    for (const path in modules9) {
    ...
    const modules12 = import.meta.glob('./components/ai/*.vue');  // ai
    for (const path in modules12) {
    ...
    for (const path in modules13) {
     ...
     const modules14 = import.meta.glob('./components/node/*.vue');  // node
    for (const path in modules14) {
      const module:any = await modules14[path]()
      components.value[path.slice(0, -4)] = module.default
    }
})
