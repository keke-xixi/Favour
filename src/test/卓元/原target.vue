...
const getPermission = async () => {
    switch (state.clean) {
        case ROLE_TARGET_CONFIG.clean_leader.value:{  // "1"
            state.targetKey = "clean_leader"
             const result = await getTargetInfo();
            console.log(result,'clean_leader')

            if (targetType.value === 'person_target') {
                const { completedOrders,completionRate,ontimeCompletedOrders,ontimeCompletionRate,overtimeUnsolvedOrders,statsDate,totalOrders } = result || {};
                state.timeoutNum = overtimeUnsolvedOrders || 0; // 超时未解决单数量

                // 完成率
                const row = state.targetList.find(item => item.target === '完成率')
                if(row) {
                    row.percent = formatValue1(completionRate);
                    row.rate = formatValue(completedOrders) +'/' + formatValue(totalOrders);
                    console.log(row)
                }

                // 按时完成率
                const ontimeRow = state.targetList.find(item => item.target === '按时完成率')
                if(ontimeRow) {
                    ontimeRow.percent = formatValue1(ontimeCompletionRate);
                    ontimeRow.rate = formatValue(ontimeCompletedOrders) +'/' + formatValue(totalOrders);
                }
            } else {
                const {
                    deptCompletedOrders, deptCompletionRate, deptOntimeCompletedOrders, deptOntimeCompletionRate,
                    deptOvertimeUnsolvedOrders, statsDate, totalOrders, weekDeptReworkRate,
                    deptNoOperationRate,deptNoOperationUsers,deptTotalUsers, weekDeptTotalOrders
                } = result || {};
                state.timeoutNum = deptOvertimeUnsolvedOrders || 0; // 超时未解决单数量

                // 完成率
                const row = state.targetList.find(item => item.target === '完成率')
                if(row) {
                    row.percent = formatValue1(deptCompletionRate);
                    row.rate = formatValue(deptCompletedOrders) +'/' + formatValue(totalOrders);
                }

                // 按时完成率
                const ontimeRow = state.targetList.find(item => item.target === '按时完成率')
                if(ontimeRow) {
                    ontimeRow.percent = formatValue1(deptOntimeCompletionRate);
                    ontimeRow.rate = formatValue(deptOntimeCompletedOrders) +'/' + formatValue(totalOrders);
                }

                // 返工率
                const rewordRow = state.targetList.find(item => item.target === '返工率')
                if(rewordRow) {
                    rewordRow.percent = formatValue1(weekDeptReworkRate);
                    rewordRow.rate = formatValue(weekDeptReworkRate) + '/' + formatValue(weekDeptTotalOrders);
                }
            }
            console.log('state.targetList', state.targetList)
        ...
        case ROLE_TARGET_CONFIG.cleaner.value: { // "2"
            state.targetKey = "cleaner"
            const result = await getTargetInfo();
            console.log(result,'cleaner')
            const { completedOrders,completionRate,ontimeCompletedOrders,ontimeCompletionRate,overtimeUnsolvedOrders,statsDate,totalOrders } = result || {};
            state.timeoutNum = overtimeUnsolvedOrders || 0; // 超时未解决单数量
            ...
            if(ontimeRow) {
                ontimeRow.percent = formatValue1(ontimeCompletionRate);
                ontimeRow.rate = formatValue(ontimeCompletedOrders) +'/' + formatValue(completedOrders);
            }
            break;
        }
            
        case ROLE_TARGET_CONFIG.clean_manager.value:{ // "3"
            state.targetKey = "clean_manager"
             const result = await getTargetInfo();
            console.log(result,'clean_manager')
            console.log(state.targetList,'state.targetList')
             const { deptCompletedOrders,deptCompletionRate,deptOntimeCompletedOrders,deptOntimeCompletionRate,deptOvertimeUnsolvedOrders,statsDate,totalOrders,
                    deptReworkRate,deptReworkOrders,deptNoOperationRate,deptNoOperationUsers,deptTotalUsers
             } = result || {};
            state.timeoutNum = deptOvertimeUnsolvedOrders || 0; // 超时未解决单数量
            ...
            const rewordRow = state.targetList.find(item => item.target === '返工率')
            if(rewordRow) {
                rewordRow.percent = formatValue1(deptReworkRate);
                rewordRow.rate = formatValue(deptReworkOrders) + '/' + formatValue(totalOrders);
            }

            // 人数占比
            const noOperationRow = state.targetList.find(item => item.target === '人数占比')
            if(noOperationRow) {
                noOperationRow.percent = formatValue1(deptNoOperationRate);
                noOperationRow.rate = formatValue(deptNoOperationUsers) +'/' + formatValue(deptTotalUsers);
            }
            break;
        }

        case ROLE_TARGET_CONFIG.environment_manager.value: {  // "4"
            state.targetKey = "environment_manager"
            const result = await getTargetInfo();
            console.log(result,'environment_manager')
            const { deptCompletedOrders,deptCompletionRate,deptOntimeCompletedOrders,deptOntimeCompletionRate,deptOvertimeUnsolvedOrders,statsDate,totalOrders,
                    deptReworkRate,deptReworkOrders,deptNoOperationRate,deptNoOperationUsers,deptTotalUsers
             } = result || {};
            state.timeoutNum = deptOvertimeUnsolvedOrders || 0; // 超时未解决单数量
        ...
        case ROLE_TARGET_CONFIG.project_manager.value:{  // "5"
            state.targetKey = "project_manager"
            const result = await getTargetInfo();
            console.log(result,'project_manager')
            const { deptCompletedOrders,deptCompletionRate,deptOntimeCompletedOrders,deptOntimeCompletionRate,deptOvertimeUnsolvedOrders,statsDate,totalOrders,
                    deptReworkRate,deptReworkOrders,deptNoOperationRate,deptNoOperationUsers,deptTotalUsers
             } = result || {};
            state.timeoutNum = deptOvertimeUnsolvedOrders || 0; // 超时未解决单数量
    ...
    const menu = await api.getMetricOpertionControlMenu({ clean: state.clean })
