        ...
        <view class="container-top" ref="containerTopRef">
            ...
            <view class="function-cards">
                <view class="card-box card-required" @tap="goToRequiredTask">
                    ...
                    <view class="card-title" :style="{ fontSize: fontSizeMedium }">必查项</view>
                    <view class="card-desc" :style="{ fontSize: fontSizeMini  }">共{{ state.mustCheckNum }}单，已查{{ state.checkedNum }}项</view>
                </view>
                <view class="card-box card-optional" @tap="goToOptionalTask">
                    <view class="card-icon">
                        <image src="/static/image/kcd.png"></image>
                    </view>
                    <view class="card-title" :style="{ fontSize: fontSizeMedium }">可查项</view>
        ...
        <custom-scroll 
          ...
          >
                ...
                <view class="InfosPageRef">
                  <InfosPage :id="infoData.id" @getInfoData="getInfoData" ref="inforef" />
                </view>
                ...
                <view class="BillUnMustRef" id="BillUnMustRef">
                    <view class="bills-container">
                        ...
                        <view class="header">
                            <view class="title">
                                <view class="title-text" style="color: #1BC575;">可查项</view>
                            </view>
                            <view class="toggle-arrow" @tap="toggleExpandUnMust">
                                <view class="expand-text" :style="{ fontSize: fontSizeMini, color: '#9c9c9c', fontWeight: 500 }">{{ isExpandUnMust ? '收起' : '展开' }}</view>
                                <uni-icons :type="isExpandUnMust ? 'up' : 'down'" :size="24" color="#9c9c9c"></uni-icons>
                            </view>
                        </view>

                        <!-- 可查单列表 - 根据isExpandUnMust控制显示 -->
                        <view class="list" v-show="isExpandUnMust">
                            <view 
                                v-for="(item, index) in state.unMustList" 
                                :key="index" 
                                :id="'unMustId' + index"
                                class="card-box"
                                :style="{ fontSize: fontSizeMedium }"
                            >
                                <BillUnMust :item="item" :index="index" :showBtn="!disabledBtn" @report-list="refreshData" />
                            </view>

                            <!-- 空状态提示 -->
                            <view class="empty-state" v-if="state.unMustList?.length === 0">
                                <text class="empty-text" :style="{ fontSize: fontSizeMedium, }">暂无可查单数据</text>
                            </view>
                        </view>
                    </view>
                </view>
                ...
                <!-- 执行计划扫描码 -->
                <ExecutionPlanScanCode v-if="spaceIds" :disabled="disabledBtn" ref="execution" :workOrderSpace="workOrderSpace"
                ...
                <!-- sop执行计划 -->
                <ExecutionPlan ref="execution" v-if="workOrderSpace.length > 0 && !spaceIds" :disabled="disabledBtn"
...
defineOptions({
	behaviors: ['uni://proxy-scroll-view'],
})
// 添加生命周期钩子
onShow(() => {
...
const toggleExpandUnMust = () => {
  isExpandUnMust.value = !isExpandUnMust.value
}
...
const goToRequiredTask = async () => {
  ...
  scrollRef.value?.setTargetId('BillMustRef')
};
...
const goToOptionalTask = async () => {
    ...
    scrollRef.value?.setTargetId('BillUnMustRef')
};
          ...
          if (indexMust !== -1) {
              isExpandMust.value = true;
               setTimeout(async () => {
                  ...
                  // console.log('moveDistance1',indexMust,moveDistance)
