...
class WebSocketClient {
    ...
    connect() {
        try {
            ...
            const fullUrl = this.url + '?secretKey=ws_root&secretPassword=ws_power123&uid=' + socketUid;
...
// WebSocket 服务地址
const wsUrl = "ws://192.168.232.110:56005/api/websoket/subPoint"

// 需要订阅的测点列表（原 data.data）
const pointsList = []
    ...
    if (data && data.data) {
        ...
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                const pointValue = obj[key]?.value
                if (key.includes('431125JYZJFDSYZ01YX')) {
                    ...
                    list.push({
                        id: key,
                        text: pointValue !== undefined && pointValue !== null ? pointValue.toFixed(2) : '0.00'
                    })
                }
            }
        }
        ...
        meta2d.render()
        console.log('WebSocket 数据已更新', list)
    } else {
        console.warn('收到非预期格式的 WebSocket 消息', data)
    }
})
